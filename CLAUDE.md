...
**That grep answers with more files than the count, and always will.** It returns **thirteen** against a count of eleven, and the two extras are extras for two different reasons — neither of them a pin, and neither anything to update when the year rolls over:

- **`playerSplits.ts`** answers the `hfSea` half of the pattern while *importing* `SEASON` from `research.ts` rather than declaring one. A **use**, and it says so in a comment above its `hfSea`.
- **`percentileCuts.ts`** answers it from **prose alone** — the word `hfSea` appears in a comment explaining that a cut card is the current season's or nothing, and nowhere else in the file. It imports `SEASON` too and declares nothing. Left as it is deliberately: a reader auditing the roll-over *should* meet that file, because the card it builds is pinned to this season on both of its halves and silently answers for the wrong one if either moves.

So the reading is: thirteen files answer, eleven of them declare, and the two that don't each say why in the comment that made them answer.
...
| `MlbView.tsx` `MlbScoreboard.tsx` `MlbStandings.tsx`, the main tab strip | `sicko-mlb` |
