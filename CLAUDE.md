...
- **Client**: `PitcherCard.tsx` renders, in this order: the **game line** (`GameLine`), the batters faced **grouped by inning**, then the **arsenal**. The game line deliberately reuses the arsenal row's parts instead of `StatPill`s so the two read as one table — a `.pline` caption, then an `.ars-row` accented by `decisionColor()` whose head carries the W/L/S, IP and pitch count (with strike% in the usage bar), whose `.ars-metrics` grid holds H/R/ER/BB/K/HR, and whose `.ars-results` strip holds ERA/WHIP/Whiff/CSW. (A header **W/L/S** chip still sits in the card header.) The arsenal itself is a **Savant-style color-coded table** in a collapsible `ArsenalSection` (one row per pitch type: `pitchStyle()` dot+abbr+usage bar, velo/spin/iVB/HB with ▲▼-vs-season deltas + whiff, then a **Szn vs** Results strip of BA/SLG/wOBA/xwOBA/PutAway). The `InningBlock`s are **collapsed by default** (header shows BF/H/R/ER/K/BB/pitches — `inningStats()`; ER only when it differs from R) and expand to the result rows, each of which expands again to the full pitch-by-pitch sequence. That sequence (pitch table + `StrikeZone` plot, sharing a hover/tap highlight) lives in the shared `PitchSequence.tsx` — extracted from `PlateAppearanceCard`, which now reuses `PitchTable`. Every expandable here (inning, faced batter, arsenal) mirrors the batter `.pa-card`: same radius/left accent, hover tint, focus ring, caret-sideways-when-closed, and `useScrollIntoViewOnExpand` so opening one brings it to the top. The batter-name font shrinks on narrow cards via `@container` queries on `.faced-batter`. `SummaryTable` renders a pitcher sub-table; `LiveFeed` interleaves faced-batter entries + a `'pitching'` live role; `PlayerDetails` gains an `isPitcher` prop.
