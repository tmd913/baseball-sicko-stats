...
- **Client**: `PitcherCard.tsx` renders the aggregate `StatPill`s (incl. ERA/WHIP via `eraOf`/`whipOf` and a colored W/L/S **Dec** pill, plus a header **W/L/S** chip), a **Savant-style color-coded arsenal table** in a collapsible `ArsenalSection` (one row per pitch type: `pitchStyle()` dot+abbr+usage bar, velo/spin/iVB/HB with ▲▼-vs-season deltas + whiff, then a **Szn vs** Results strip of BA/SLG/wOBA/xwOBA/PutAway), and the batters faced **grouped by inning** into `InningBlock`s that are **collapsed by default** (header shows BF/H/R/ER/K/BB/pitches — `inningStats()`; ER only when it differs from R) and expand to the result rows, each of which expands again to the full pitch-by-pitch sequence. That sequence (pitch table + `StrikeZone` plot, sharing a hover/tap highlight) lives in the shared `PitchSequence.tsx` — extracted from `PlateAppearanceCard`, which now reuses `PitchTable`. The batter-name font shrinks on narrow cards via `@container` queries on `.faced-batter`. `SummaryTable` renders a pitcher sub-table; `LiveFeed` interleaves faced-batter entries + a `'pitching'` live role; `PlayerDetails` gains an `isPitcher` prop. (Unrelated: `.player-nav.is-stuck` now paints an opaque background so cards scrolling under the stuck strip don't show through.)
