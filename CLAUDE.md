...
**That grep used to answer with more files than the count and does not any more.** It returned **thirteen** against a count of eleven for as long as `playerSplits.ts` and `percentileCuts.ts` existed — the first answering the `hfSea` half of the pattern while *importing* `SEASON`, the second answering from **prose alone**, the word `hfSea` appearing in a comment and nowhere else in the file. Both went with the percentile card's cut control (every comparison it offered is a Splits-tab card now), so the grep answers **eleven** against a count of eleven and every file it names declares a pin.

**Expect that to stop being true again.** A *use* of `SEASON` in a Savant parameter, or a comment mentioning `hfSea`, will put a file back in the filename grep without adding a pin — which is why the arithmetic below, and not the filename count, is the check.
