...
export interface ScheduleGame {
   ...
   *
   * It is what a day's games are **ordered** by on the server, and that is what
   * it is on the wire for rather than for the label it also buys. `gamePk` is
   * not an order: measured over the 2026 season's 44 doubleheader club-days, id
   * order and game order **disagree on 30 of them**, a makeup added to a date
   * taking a fresh and higher id while being the opener. First pitch agrees with
   * this field 44 of 44.
   */
  gameNumber: number;
  /** ISO first pitch, or null where the schedule gives none. */
