...
**And the map is keyed by player key rather than by MLB id, because a seat has a side of the ball.** ESPN files a two-way player as **one** entry — Shohei Ohtani, eligible `DH, SP`, standing in one slot on any given day — where the app draws him as **two rows under one id**, `batter-660271` and `pitcher-660271`. Shipped as ids, his single `UTIL` afternoon answered *started* for both of them, and his pitching row drew above the pitching table's `Lineup` divider on a day he was seated as a hitter. `seatKinds` is the rule (`espn.ts`, mirrored into `lib.ts` for the client's own end-of-range answer): a one-kind player is himself in whatever chair he is in, and a two-way player belongs to `pitcher` in slots 13/14/15 and to `batter` in every other — the fail-safe direction `BENCH_SLOT` and `IL_SLOT` already take, and the same three ids `projection.ts` splits its seats on, which is why that set is `espn.ts`'s export now rather than a second copy. Reading it costs nothing: `startedKeys` derives the map from **each day's own roster**, slot included, so the answer is per day — a man at `P` on the Monday and `UTIL` on the Tuesday is two different rows' start. See **Client — the Roster view**, *A two-way player is two rows, and the seat he is in is only one of them*, for what it moves on the page and the totals it leaves alone.
