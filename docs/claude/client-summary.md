...
**And the fourth label is `In game` now, which is the third thing that entry has been called.** `Pitching` was the wrong *grammar* and the paragraph above is the record of catching it; `On mound` was the wrong **fact**, which is worse, and the legend is again what made it visible — four labels in a row, three of which say where a man is standing and one of which says where he is not. `liveRoleGame` reads `GameStatus.inGamePitcherIds` rather than `pitchingId`, deliberately and for a measured reason (see the feed reference: a starter dropped out of the Live section every time his own side came up to bat and climbed back in an inning later, all night). What that makes the role mean is **still in the game** — so for half of every game the man wearing `On mound` is in the dugout with a jacket on. The claim about the mound survives where it is true: `pitchingId` keeps its meaning and its one reader, the `.inning-block.active` accent, which is a statement about the half being thrown *now*.

**It costs nothing and gives back more than the last rename took.** Measured on the same feed pill the 78.05 → 86.14 above was read off, under `sim=1` at 1440: `Pitching` **78.05**, `On mound` **86.14**, `In game` **70.03** — the shortest of the three. The legend itself is unchanged at **one 37px row**, drawing `At bat · On deck · On base · In game`, and page overflow is 0. Nothing else about the entry moves: the swatch is still `role-pitching` reading `--role-pitching-bg`, and `--mound-teal` keeps its name, a token being the name of a color rather than of a caption.
...
**A third reader has since joined them, and it is not a view at all.** The research board's `Starting` filter — *show me the pitchers due to start on these days* — reads this module too: `turnsOnDays` asks `startTierOn` per game, which is the test the `GS` column sums and the day cell draws its box with, so the filter and the grid cannot disagree about whether a man is up on Friday. That is the whole reason it reads the index rather than a start-day map of its own. It takes one of its **own**, `buildTurnIndex` — this builder at `TURN_DAYS` (14) with no matchup — because the days it offers must not move with the span the grid happens to be on, or with whether the grid is on at all. It carried a fourth span value (`'window'`, over all 28 days) for one commit, until the strip came down to a fortnight and the run turned out to be a span this file already had a name and a number for. The whole of that control is in **Client — the research board**, *The turn filter*; what is here is that the fact it selects on is this file's.
