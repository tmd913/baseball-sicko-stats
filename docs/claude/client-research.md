...
**Their `Δ1d` is a number on day one and their `Δ3d` and out are dashes**, which
is not this file's business but is what a reader sees. The trend is a diff of a
daily snapshot and the first snapshot that can name these men is written the day
the join reaches them, so an absent baseline is *withheld* rather than read as a
rise from nothing — except for the one-day window, where ESPN's own published
change supplies the missing baseline. Both halves are in **The pool's own join
is extended** in `espn.md`. Driven on the board, `Δ1d` descending with every
include on: **Tristan Peters +8.4, Walker Jenkins +6.0, Jac Caglianone, Daylen
Lile, Connor Norby +2.1**.
...
A drag's order is held locally while it is live and **committed on release**, unlike the editor's row list which moves as it goes — the difference is what is downstream, twenty rows there against six hundred players by twenty-five columns here. A press commits once, on the press that places. What the row hands back is **threaded through the saved list rather than replacing it** (`reorderColumns`): a saved list can name a column this board cannot resolve right now — `rosterPct` before the ESPN status lands, a trend window with no baseline — and those never reach the order row, so committing it as it comes would drop them from the reader's preference outright (checked: a `cols=` naming `rosterTrend30` on an install with no 30-day baseline still carries it after a press-and-press reorder).
  ...
  - **`Ranks` draws a percentile under every value** (`components/columnRanks.tsx`, the toggle last in the tools run). A number on a research table says what a player did and not whether it is any good — `.265` is a fine batting average, a poor xwOBA and an extraordinary barrel rate — and the badge answers that in one figure per cell: **0–100, and 100 is always the good end**. The rule, the population and the badge itself live in that one file, which both this board and the player page's Stats tab read; see **The Stats tab** in the player-page notes for how that tab reaches the same population from a page which has none of its own.
    ...
    **The badge is a second line rather than something beside the value**, and the width is why. This board carries 23 to 44 columns, already overflows a 1920px screen, and keeps its 13px gutter where the two other wide tables grew theirs on exactly this arithmetic — "a pixel a side is 54px of scroll on a 27-column board" — so an inline badge at twenty-odd pixels a cell is five hundred across a default board and two more stat columns off the right edge of a phone. Under the value it costs the table almost nothing across and **nothing down at all**: the row is 58px, set by the 42px headshot (6 + 46 + 6 against a text cell's 12 + content + 12), and the `Opp` cell already draws two lines inside the 34px that leaves. It was **monochrome** in `--faint`, the tone the board already gives the quieter of two lines in a cell (`.research-opp-sp`), because this table's standing rule is that "the stat columns are monochrome … color is reserved for *state*" — a heat scale would be a second color system beside the live inning, the postponement and the trend. The rule was scoped to `.research-table td .col-rank` so it outranks the **sorted** column's accent: checked, a badge inside a cell computing `rgb(56, 189, 248)` computed `rgb(92, 111, 151)`.

    ***That is reversed, and the reversal is worth reading.*** **The badge is a filled chip on every table that draws one now** — this board (both readings), the player and team pages' Stats tab, the League Rankings and the opponent table — filled from one `fillAt` in `columnRanks.tsx`, with `--rank-hot` (`--strikeout`, red) at the good end and `--rank-cold` (`--walk`, blue) at the other, which are the percentile card's own two ends. **What the monochrome argument got wrong is which thing is colored**: the rule it quotes is about the *values* — `OPS` in accent made the eye jump between columns of numbers — and the values are still monochrome on every one of these tables. A rank badge is not a value; it is a quiet second line whose whole job is to say *is that any good*, which is a **scale**, and `RULES.md` already names a rank badge as the case where a scale genuinely is the reading. It was also already drawn colored on two of these surfaces (the League table and this board's own **team** reading), which made the remaining grey one a difference nothing on screen could explain — a reader crossing from the team reading to the player reading met the same badge in the same slot on the same table, one a scale and one grey. **Measured at 1400 after**: 1,226 badges on the default board, each box **14.00px** tall, rows **58.00px** — the figure the whole geometry below is built to protect — and `--text` on the chip against `--faint` off it.

    **A badge on a player outside the population wears a dashed underline**, and the whole of the argument is that this is *not* a second meaning for a broken border. The app's ladder is solid = measured, broken = ours, and the percentile card one tab over already draws a dashed ring on **exactly these men**: Savant publishes no `percent_rank_` for a player under its bar, so `percentiles.ts` ranks him against the qualified distribution itself and marks the bar `estimated`, which the card draws as `.pct-bubble--est`. Driven and counted on the live app: an unqualified batter's card (Aaron Judge, 261 PA) draws **32 of its 39 bubbles ringed**, where a qualified one's (Yordan Alvarez) draws 4 of 41 — the four being rows Savant ranks for nobody. Same set of players, same sentence — *the league publishes no standing for this man, so this placement is ours* — so the board says it in the same **vocabulary**: the same `dashed`, the same `--faint`.

    **The shape differs, and only the shape.** *(This paragraph claimed the stronger thing for a while — that the two "agree mark for mark" — and the badge was a dashed **ring** for exactly that reason. It is an underline now. The sentence is left as it was written above and corrected here, this file's rule for its own superseded reasoning; what it got right is that the two marks are one claim, and what it over-reached on is that one claim has to be one drawing.)* A ring round a colored bubble is a mark on a bubble. There is no bubble in a table cell — a closed ring there is a second **box** under a full-size value, on the app's widest table, in a column where qualification alternates row to row, and 325 of the 1,250 badges on a default board wear it. Counted at 1400 on the free-agent batting board: **325 of 1,250**, which as rings is 325 little frames down eleven columns and reads as a checkerboard rather than as a mark on a number. An underline is the least that can carry the claim, and it carries it in the one place the reader is already looking.
    ...
    **`--faint`, not `--text`.** The percentile card's ring is `--text` because it sits on a colored bubble; here the badge *is* `--faint`, the tone this table gives the quieter of two lines in a cell, and a mark louder than the number it belongs to would invert the one hierarchy the cell has. *(Superseded by the chip: the badge is `--text` on a tinted ground now, so `--faint` is a third tone rather than the badge's own ink, and a grey dash under white digits reads as a smudge. The underline takes `currentColor` on the board and the Stats tab — which is this same argument re-derived on the new ground, the line matching the number it is about. On the two tables that draw a **standing** the modifier is unreachable: thirty clubs have no bar to be short of.)* **A `text-decoration`, not a `border`** — *(this read "an `outline`, not a `border`" while the mark was a ring, and the constraint it names is unchanged)* — for the reason `.pct-bubble--est` gives and `.sched-vs-estimated` gives: a border is part of the box, so a mark that comes and goes down a column would change a row's height and a table's width by which players happen to qualify. A decoration is painted on the *text* and lays out nothing at all, which is the same guarantee by a shorter route. Every `.col-rank` on these two tables also takes `width: fit-content` and an auto start margin, so the box hugs its digits — that was written for the ring and the underline no longer needs it, but its other two readers do: the shape `.league-table td .col-rank` gives its own pill, and the badge landing on the exact edge the value above it sits on. **Measured on the same rows with the modifier toggled in the DOM**: at 1400 the table is `2361.05px` with the mark and without it, every row `58.00px` either way, and each badge's box `7.31px`. (The ring was measured the same way at 390 / 1200 / 1920 — table width `1583.25 / 1998.70 / 1998.70`, rows 58.00, page overflow 0 — and the underline moves nothing the ring did not move either.) Computed on a marked badge: `underline dashed rgb(111, 115, 117)` at `1px` with a `2px` offset, and `outline-style: none`.
