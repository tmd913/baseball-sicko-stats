...
**The card has two arrangements now, and only one of them is ours.** `SUMMARY_SECTIONS` / `PITCHER_SUMMARY_SECTIONS` are **Savant's own card** — the fifteen bars its player page draws, against the thirty-odd of the tables above — and they are *measured, not remembered*: Savant renders that card client-side, so the metric list is nowhere in the HTML this file already scrapes, and it was transcribed out of the page's own bundle (`/sections/player-update/.../scripts/build/index.js`, five group arrays for a batter and two for a pitcher, assembled by `U5`/`sle`/`cle`). The alternative was writing a card down from memory and having it drift silently from the page it claims to match. Three things came out of that transcription and are worth keeping: Savant's `inverse` flag is **exactly this file's `lowerBetter`** (its `L5` uses it for one thing, flipping the z-score when it estimates off `metricSummaryStats` — the same job); its summary card ranks xERA by `percent_rank_xwoba` rather than `percent_rank_xera`, which reads as a bug and is not one, the two fields carrying the **same number** (measured across the range: Sánchez 78/78, Skubal 1/1, Cole 80/80, Skenes 90/90, Nola 44/44), so this table names the field named for the stat; and the rows Savant *shows but does not rank* — every `BATTER_COMPUTED` and `PITCHER_COMPUTED` entry — are **not** on the summary card, so it is built with `extras: false`. That last is not a simplification: splicing by section title would land HR at the foot of `Batting` (there being no `xhr` there to sit ahead of) and ERA in `Value`, which is a card Savant does not draw. Both arrangements come off **one scrape of one row** — one more pass over a fifteen-entry table, no second request, leaderboard or blob — which is what lets the client's density switch be a render rather than a round trip, and what stops the two ever disagreeing about a number. `summary` is a field added to the stored blob *and read straight back out of it*, which is both halves of the bump test, so `CARD_VERSION` is **6**.

> **Gone.** Everything from here to the end of the cut-card discussion describes `percentileCuts.ts` and `playerSplits.ts`, which are **deleted**. The percentile card has no cut control: every comparison it offered — the two hands, the two parks, recent form, and finally the two halves of the season — is a card on the **Splits** tab, where both columns are drawn at once with the measured gap between them, which is the reading a comparison wants and the one a one-sided card could never give. So the card is his season's and every bar on it is Savant's own rank rather than one of ours drawn broken. What follows is kept for two things it still establishes: the join it *refused* to make (ranking a value computed by one party inside a distribution computed by another), and the noise measurement below, which is the reason any cut of a season is a worse population than the season.
>
> Deleting them also took the `cut=` parameter off `/api/percentiles/:playerId` and `/api/players/:playerId/windows`, and let `statcastWindow.ts` un-export `empty`, `tally` and `toStatcast` — module-local again, their one outside caller having been `playerSplits.ts`.

**A cut of the season is a different card built a different way, and `percentileCuts.ts` is the whole of it.** The question is *is he a different hitter against left-handers* and the answer that makes it comparable to anything is: his value **in the cut**, placed where that value would put him **among all qualified players over the full season**. Not ranked within a population of splits — Savant publishes no split leaderboard, and there is a better reason than availability, since ranking splits among splits scores a merely-average platoon hitter 50 twice over and says nothing. Savant's `percent_rank_` fields exist for the season and for nothing else, so **every bar on a cut card is ours** and every metric on one is marked `estimated` and drawn broken: the standing rule that an estimate never wears the same clothes as a measurement, applied to a whole card rather than to a row.

**Both sides of that comparison come off the same board**, which is the property that makes it honest rather than approximately honest. The cut's values are a `ResearchRow` from `playerSplits.ts` (one player's season of pitches through `statcastWindow.ts`'s own `tally`/`toStatcast`); the population is `getResearch(kind, 'season')` filtered to `qualified`, the very flag and the very population the board's own rank badges use — so a reader never meets two different percentiles for one number on two surfaces. It is therefore drawn in **board units throughout and never mixed with the scraped card**: the season card's `Avg Exit Velocity` is Savant's `exit_velocity_avg` off the player page and a cut's is `exitVelocity` off the board, and ranking the first inside a distribution of the second would be comparing two numbers that mean the same thing and are computed by two different parties. `CUT_SOURCE` — keyed by the metric's key **on the season card** — is what keeps the two the same card: a metric with an entry is drawn, one without is not, and the sections and their order are otherwise the season card's own. What that costs a cut is the run values, everything fielding and catching, sprint speed, and the handful of rows with no board column behind them; those are **dropped rather than dashed**, because a dash on the season card means *Savant has no number for this man* and a row no cut of anybody could fill is a fact about the shape of the card. One trap found by drawing it: a batting average is `avg` on a batter row and `avgAgainst` on a pitcher's, and reading only the first cost the pitcher card its BA row *silently* — the metric resolved to null and the drop rule took it away, which is the failure mode that looks like a design decision.
...
- `cbs-index-v1.json` — **CBS Sports' whole player list reduced to what the news enrichment needs**: MLB id → CBS's numeric player id, **966 pairs in ~15KB**, on a 6-hour window in memory and in the storage tier with an `inFlight` guard — the RotoWire index's own shape, TTL and guard, because it is the same kind of thing. What it is reduced *from* is **31 requests and ~16MB** (thirty club roster pages plus the league-wide injuries page), which is 24× the bytes the RotoWire index costs, and is why the blob matters more here than there: *store the answer, not the payload*, at its most literal. **An id and not an address**, unlike RotoWire's slug — a CBS player URL with the right number and no slug at all is a 200, and with a *wrong* slug still a 200, so nothing about a rename can break a stored row. The index is built from the roster pages' schema.org `athlete[]` blocks, which carry full names and hand the club over for free; the injuries page carries neither and its rows are matched on a slug-derived name alone, which is where all 93 of the unmatched go. See **Client — the player page's tabs**, *The analysis was behind a wall*, for what it buys and why CBS is an enrichment of RotoWire's list rather than a replacement for it.
...
**`getDay(date, filter?)`** takes an optional `DayFilter` (`dayFilterFor(players)`). A day holds a report for *every* player who appeared — ~600, several MB — so `getReport` narrows each day to the rostered players as it parses. The filter carries **names as well as ids**, because `findPlayerDay` falls back to a same-kind `savantName` match when an id isn't present that day; filtering on ids alone would silently break that. Frozen, filtered days are memoized in a bounded `projectedCache` keyed by date + roster; only still-mutable days are kept whole in `memCache`. `mapLimit` (`limit.ts`) caps the fan-out at `DAY_CONCURRENCY` (6) and `GAME_CONCURRENCY` (8) — this bounds peak heap as much as it bounds sockets. **And `getDay` dedups in flight**, keyed exactly as `projectedCache` is: the memo only helps once the answer exists, so *N* concurrent calls for one day all missed, all gunzipped the same 350–860 KB snapshot and all parsed the 5–8 MB inside it. Measured on the Overview, whose eight day-reads overlap by construction: **sixteen calls, sixteen reads, eight distinct days**. Concurrent callers already shared the objects — `projectDay` copies the Map and not the `PlayerReport`s in it — so this extends an existing sharing rather than introducing one. **`getReport`'s `parseWith` is the other half**: a caller making several reads over one window can hand them all a single, wider parse set so they are one question rather than four, which changes the parse and never the answer (`players` still decides the rows, `held` still decides the days). `/api/overview::overviewParseSet` is the only caller; see *The server*, *The long-span Overview*.
...
### One player's two halves — and why they had to come off his own pitches *(deleted)*

`playerSplits.ts` is **gone**, with `percentileCuts.ts`, its last reader. The
percentile card has no cut control at all now, so nothing asks for a Statcast cut
of a season and the per-player season-of-pitches fetch below has nobody to serve.
The section is kept whole: it is the record of four upstream dead ends and of a
reconciliation that took real work to establish, and the next person who wants a
windowed or split Statcast line should meet it before writing the fetch again.

*(What follows was written when it built two halves, and describes the four-split
build before that.)* **It built four splits over five spans and builds two
halves**:
