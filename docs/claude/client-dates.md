...
**A selection is a claim, so `DateCalendar` can carry none.** `start` and `end` are `string | null`, with an optional `month` beside them saying which month to open on where there is nothing picked. Every caller here passes a range it genuinely has and none of them changed; the reader is the research board's projected span picker, which opens a calendar over days the reader has explicitly *not* chosen yet — `Custom` off `Week 20` means *not those days* — and used to mark **today** there, which is the calendar answering the question it was opened to ask. Nothing is marked now, the foot says what the first press does (`Pick a day, or the first of a range`) instead of naming a day nobody chose, and today is still the month the grid opens on, which is a different claim and a true one. See **The research board**, *The panel reopened blank, and the calendar left the head*.
...
**`fixed` on `DateBar`**: no arrows, and the face is a `<div>` rather than a `<button>`. It exists for a matchup team page's **Summary** reading, whose span is the period's start to today — **and for the app's own Roster now**, which grew the same reading (see **Client**, *`Summary` is the Roster's fifth reading*). Two surfaces, one bar, one `{ kind: 'matchup' }` face.

**That second caller is what took `!fixed` out of `usePublishedHeight`'s test.** It read `measure && !fixed`, which was true rather than load-bearing while every fixed bar was a team page's: `measure` already says *this is the bar a page's header row sticks under*, and the second clause was a second copy of that claim reading a prop about something else. With it in, the app's own bar stopped publishing `--date-bar-h` at the exact moment it changed shape, leaving the summary table's header row stuck under the height of a bar that is no longer there. `barRef` is attached in the fixed branch with it. Measured on the Roster crossing the toggle: **`--date-bar-h` is 54px in both readings**, so nothing under it moves on the press.
