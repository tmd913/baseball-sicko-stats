...
### The tab strip stops scrolling once it fits, and points at what it hides

**Two things were wrong with it and they were the same thing seen from either
end.** `.details-tabs` was capped at the reading column's **680px**, so a strip
whose labels come to **747px** for a batter and **832** for a pitcher was a
scroller on a 1,920px monitor with a thousand pixels of empty page either side
of it — and nothing on screen said so, the scrollbar being deliberately hidden
(`scrollbar-width: none`, so a horizontal bar does not eat a tab's underline).
The only hint that `Charts` existed was a half-tab peeking at an edge, and that
is there only when the strip happens to be scrolled to a place where a tab
straddles one.

**680 is a column for cards and a control is not a card.** Six of the tabs open
onto a 680px card, which is what that number is for; the strip *names* those
cards. So the cap becomes a floor: `max-width: max(680px, var(--tabstrip-grow))`,
and where the page has room the strip takes its own width. It is still centered
and the chrome's rule still spans the wider box, which is what it was already
doing — that rule was never the 680 column's.

**`--tabstrip-grow` is measured, because there is no one number.** Whether the
tabs fit is a function of the viewport, of *which* tabs a kind of player has (a
pitcher's Arsenal tab is 79px of the difference) and of **a font this app does
not choose**. That is `--chrome-h`, `--clip-w` and `--research-head-h`'s own
rule, and a media query at whatever width the labels happened to overflow on the
machine this was written on is the fault those three exist to record.
...
it, so a strip grown to 832px for a pitcher would go on reporting 832 for a
batter whose eight tabs come to 747 — a measurement that can only ratchet
upwards, leaving a centered strip 42px off center. The tabs are `flex: none` and
`white-space: nowrap`, so their own widths are independent of the box, which
makes the sum the one figure that is stable under what it is used to set. Proved
by taking a tab out of the flow and forcing a render: `--tabstrip-grow` goes
**747px → 585px** (the 161.83px `Percentile Rankings` label and its 6px gap),
where a `scrollWidth` reading stays at 747 — and the wrapper falls back to
**680**, the floor holding.

**The arrows are laid out before they are shown.** Both appear the moment the
strip overflows, and the one with nothing behind it is `visibility: hidden`
rather than absent — *reserve the box, don't move the page*. A reader who has
scrolled to the last tab and is aiming at it must not have the strip jump 28px
sideways because the right-hand arrow just went away. Measured at 390×844,
pressing the right arrow to the end and scrolling back: the pane's left edge and
width are **byte-identical at 46 / 298 in every state** — arrows
`hidden/visible` at rest, `visible/visible` mid-strip, `visible/hidden` at the
far right, and back again. They are **small aimed targets, so they keep their
hover**: the `(hover: hover)` scoping in this app is for full-width rows and
cards in a scrolling list, where a sticky tint reads as a selection, and a 28px
square you have to aim at is the opposite case.

**The scroller and the box are two elements now** (`components/TabStrip.tsx`),
because a scroller cannot hold a child that is meant to stay put. The wrapper
owns the column width and the centering; `.details-tabs` owns the scrolling and
...
nine. Measured, before → after:

| | width | wrapper | strip | arrows |
| --- | --- | --- | --- | --- |
| batter | 1200 | 680 → **747** | scrolled → **fits** | none |
| batter | 800 | 680 → **747** | scrolled → **fits** | none |
| batter | 760 | 680 → **728** | scrolls | `hidden` / `visible` |
| batter | 390 | 358 → **358** | scrolls | `hidden` / `visible` |
| batter | 320 | 288 → **288** | scrolls | `hidden` / `visible` |
| pitcher | 1200 | 680 → **832** | scrolled → **fits** | none |
| outing | 1200 | 860 → **860** | fits | none |
| outing | 320 | 288 → **288** | scrolls | `hidden` / `visible` |

The batter's strip stops scrolling at **779px** of viewport and not at a number
anybody wrote down: 747 of tabs plus the overlay's own 16px gutters either side,
which is `--table-bleed`'s number in the box that declares it. The page body
overflows by **0** at every width in the table, and the outing page's own
