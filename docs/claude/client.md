...
**And a press of the Roster's own reading run puts that run back on screen**, which the memory above is exactly why it did not. `scrollKey` is the *view*, so `Schedule`, `Projected`, `Summary`, `Matchup` and `Opponent` all share one offset and a press carries it straight across. Measured at 430×950, scrolled 900px down `.summary-scroll`:
...
A layout effect on the readings brings the row back when it is above the scrollport. **It is worth being exact about what that amounts to**: the row is the pane's *first child*, so any offset at all has it behind the pinned chrome — a pane at 40 puts `.view-tools` at viewport 59 under a 99px head — and there is no case where this moves the reader a little. Either they are at the top and nothing happens, or they are scrolled and the pane returns to 0.

So it **is** a reset, deliberately, against this section's own argument that a reading changes the numbers in the rows rather than which rows they are. That argument is still true and is not the one that decides it: a control the reader cannot see is one they cannot press again or read the state of, which is the rule the app already applies to an empty state (*name the control that caused it, and point at where it is*) and to the percentile card's density switch. `DetailsShell` does the same thing a layer down. The effect stands down when the **view** changes, which is the one case that would fight the restore — that runs on the same commit and places a remembered offset. Checked rather than assumed: leaving the Roster 700px down for the Feed and coming back still lands at 700.
