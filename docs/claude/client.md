...
The two reports read together are the specification — the run visible **and** the reader's place kept — and only pinning gives both. So `.summary-scroll > .view-tools` is `position: sticky; top: 0` and nothing touches the scroll. The band is the run at 0, the date bar at the run's own height, and the table's header row under both; all three offsets are **measured, not declared** (`--tools-h` published by `usePublishedHeight`, joining `--date-bar-h`), which is the app's rule wherever a height is a function of the width or of a font it does not choose — and this row is both, wrapping at a narrow width and drawing a different run on a league page than on the Roster.
