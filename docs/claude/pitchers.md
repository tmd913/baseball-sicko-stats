...
data change and not a relabelling: `pitchLeague.ts` now carries a per-hand table
...
already parses, taken as the majority of the season so one mislabelled row
...
colour their ▲▼ with, rather than a second opinion beside it. A metric that
table calls neutral takes **no colour at all**, which on a slider's induced
